...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "service_u473tdb", // ✅ Your service ID
        "template_v42h7hl", // ✅ Your template ID
        formRef.current,
        "8qBkFLCQV4VkyWouO" // ✅ Your public key
      )
      .then(
        (result) => {
  ...
  return (
    ...
    >
      {isSending && <Loader />} {/* 👈 Show loader while sending */}

      <motion.div
        className="max-w-3xl w-full text-center space-y-6 text-[10px] sm:text-sm md:text-base"
